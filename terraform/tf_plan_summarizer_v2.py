...
{colorize('Example Usage:', Colors.BOLD if is_color_supported() else '')}
  # Summarize tfplan.json with colors
  python tf_plan_summarizer_v2.py tfplan.json

  # Summarize and save to report.txt (colors disabled in file)
  python tf_plan_summarizer_v2.py tfplan.json -o report.txt

  # Summarize without colors in the terminal
  python tf_plan_summarizer_v2.py tfplan.json --no-color

  # Include unchanged/read resources in the details
  python tf_plan_summarizer_v2.py tfplan.json --include-no-op
"""
    )
    parser.add_argument(
        "plan_json_file",
